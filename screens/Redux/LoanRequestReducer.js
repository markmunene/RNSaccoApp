...
export default function LoanRequestReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case 'GET_APPROVED_LOANS':
      return {
        ...state,
        ApprovedLoans: Object.assign([], action.payload),
      };
    ...
    case 'DELETE_LoanRequest':
      let temploanRequest = state.loanRequest;
      let filteredLOans = temploanRequest.filter(
        loan => loan.id != action.payload.id,
      );

      return {
        ...state,
        loanRequest: Object.assign([], filteredLOans),
      };
